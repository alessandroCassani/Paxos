...
# Network functions remain the same
...
def acceptor(config, id):   
    print("-> acceptor", id)
    instances = defaultdict(lambda: {"rnd": (0, id), "v_rnd": None, "v_val": None, "timestamp": None})
        ...
        while True:
            ...
            if msg["phase"] == Message.PHASE1A:
                ...
                if c_rnd > instances[instance_id]["rnd"]:
                    ...
                    instances[instance_id]["timestamp"] = timestamp
...
def proposer(config, id):
    ...
    instances = {}
    decided_values = set()
    pending_values = []  # Simple list of (timestamp, value) tuples
    ...
    def start_new_instance():
        ...
        # Get the earliest timestamp value
        pending_values.sort(key=lambda x: x[0])  # Sort by timestamp
        timestamp, value = pending_values.pop(0)
        ...
        if value not in decided_values:
            ...
            instances[current_instance]["timestamp"] = timestamp
            
            prepare_msg = Message.prepare(c_rnd_cnt, current_instance, timestamp)
    ...
    while True:
        ...
        if phase == Message.CLIENT:
            pending_values.append((msg["timestamp"], msg["value"]))
            
            # If no instance is active, start a new one
            ...
            if len(inst["promises"]) > get_quorum() and get_quorum() != 0:
                ...
                accept_msg = Message.accept(inst["c_rnd"], inst["c_val"], instance_id, inst["timestamp"])
            ...
            if instance_id in instances:
                ...
                # Move to next instance if there are more values to decide
                if pending_values:
                    current_instance += 1
                    start_new_instance()
...
def learner(config, id):
    ...
    decisions = []  # List of (timestamp, value) pairs
    printed_up_to = 0  # Track the timestamps we've printed
    
    while True:
        msg = json.loads(r.recv(2**16).decode())
        if msg["phase"] == Message.PHASE2B:
            decisions.append((msg["timestamp"], msg["v_val"]))
            decisions.sort(key=lambda x: x[0])  # Sort by timestamp
            
            # Print any decisions that are ready
            while decisions and decisions[0][0] > printed_up_to:
                timestamp, value = decisions.pop(0)
                print(f"[{timestamp}] {value}")
                sys.stdout.flush()
                printed_up_to = timestamp
